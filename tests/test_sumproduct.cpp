...
class BinaryCodomainMap {
  ...
  typedef sample_space_iter_pair value_type;
  typedef value_type& reference;
  typedef boost::lvalue_property_map_tag category;
};
BinaryCodomainMap::value_type get(BinaryCodomainMap c, Vertex n) {
  const static std::size_t sample_space_array[] = {0, 1};
  const static std::vector<sample_space_type> sample_space_vector(
      sample_space_array, sample_space_array + 2);
...
const std::string* expected_output() {
  // Generated by output and randomly verified with the paper
  // turn on the generate_expected_output = true flag to get the output.
...
TEST(test_sumproduct, kshischang_et_al) {
  ...
  sample_space_iter_pair cdip(sample_space_static_function());
  ...
  for(boost::tie(e, e_end) = edges(g); e != e_end; ++e)
    for (sample_space_iterator cd = cdip.first; cd != cdip.second; ++cd)
