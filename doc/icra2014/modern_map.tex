...
Let $z_f$ denote the $f$\textsuperscript{th} laser range measurement when captured from pose $g_f$. 
The problem is to find the probability of all cells of the map being occupied
given all $t$ observations, $\measurements = [z_f]^\top_{1 \le f \le t}$ and
$\poses = [g_f]^\top_{1 \le f \le t}$.
\begin{align}
...
\newcommand{\remaining}{\vect{z}_{1:t-1}, \vect{g}_{1:t-1}}
\begin{align}
...
\begin{align}
  p(x_i|\measurements, \poses) &= \frac{p(z_t, g_t|x_i, \remaining)p(x_i|\remaining)}
                         {p(z_t, g_t|\remaining)}.
\end{align}
%where
...
above equation can be simplified to \cite{merali2013icra} :
...
\begin{align}
 ...
 &= \frac{1}{Z'}\frac{p(x_i|z_t, g_t)}{p(x_i)}p(x_i|\remaining)\\
 &= \frac{1}{Z'p^{t-1}(x_i)}\prod_{1\le f \le t} p(x_i|z_f, g_f)
\end{align}
...
Static world assumption, $p(z_t|\map, \remaining, g_t) = p(z_t|\map, g_t)$, simplifies \eqref{eq:recursivesol} to :
\begin{align}
 p(\map|\measurements, \poses) &= \frac{p(z_t|\map, g_t)p(\map| \remaining, g_t)}
                      {p(z_t|\remaining, g_t)}\\
              &= \frac{p(z_t|\map, g_t)p(g_t|\map, \remaining)p(\map| \remaining)}
                      {p(z_t|\remaining, g_t)p(g_t|\remaining)}
\end{align}
...
be proved that posterior evaluates to \cite{merali2013icra}:
%\end{comment}
\begin{align}
...
range measurement $p(z_f|\map_f, g_f)$ depends on the cell occupancy 
$x_i$. In this paper, we assume that laser range measurement depends on only
those cells that the laser passes through for given pose $g_f$. 
...
called a factor graph (Fig.~\ref{fig:factor-graph}).
Note that any neighborhood $n(i)$ of variable node only consists of factors,
$n(i)\subseteq F \, \forall i \in V$ and vice versa.
...
= l_i|\measurements, \poses)$, $P(\vect{x}) \equiv p(\vect{x}|\measurements, \poses)$ and $P_f(\vect{x}_f)
\equiv
p(z_f|\vect{x}_f, g_f)$.
...
are not independent but correlated as a Markov chain. They work by moving a
random walk in the high dimensional space. Metropolis Hastings is one of the
popular MCMC methods. The idea is to sample $\vect{x}$ from the target probability
distribution $P(\vect{x})$ and after sufficient number of iterations,
we should get sample that is close to the true map.

Metropolis hastings algorithm requires a \emph{transition probability} $Q(\vect{x}'|
\vect{x}^r)$, that depends on current sample $\vect{x}^r$ and guides
...
For OG problem, 
%we need to sample map $\vect{x}$ from the probability distribution $P(\map)$.
we uniformly sample from all the cells in the map and flip the state of the
sampled cell to get the proposal point $\vect{x}'$. This procedure can be
represented in the form of transient probability:
...
\end{align}
where $i$ is the (sampled) cell whose state is different in $\vect{x}'$ and $\vect{x}^r$,
...
\begin{align}
  \bpmsg{f}{i}{l_i}{r+1} &= \sum_{\vect{x}_f \in \Omega_f: x_i = l_i}P_f(\vect{x}_f)\prod_{j \in n(f) \setminus i}\bpmsg{j}{f}{x_j}{r}
...
\begin{align}
  \bpmsg{f}{i}{l_i}{r+1} &= \max_{\vect{x}_f \in \Omega_f: x_i =
l_i}P_f(\vect{x}_f)\prod_{j \in n(f) \setminus i}\bpmsg{j}{f}{x_j}{r}\\
...
\end{align}
where $\vect{x}^f = \{x_i^f\}_{i \in n(f)}$ is the optimum assignment 
...
\newcommand{\actz}{\bar{z}_f(\vect{x}_f, g_f)}
...
\end{align}
where $\actz$ is the distance of first occupied cell in $\vect{x}_f$ starting
...
% \begin{align}
%   p(z_f|\vect{x}_f, g_f) &= \frac{1}{Z}\begin{cases}
%               1 & \text{ if } \vect{x}_f = [0, 0 \dots 0, 1]^\top\\
...
%$\vect{x}^f$ to be a vector of ordered cell labels through which laser
...
\begin{align}
  p(z_f|\vect{x}_f, g_f) &= \frac{1}{Z}\begin{cases}
                       1 & \text{ if } \vect{x}_f = \vect{R}_{f1}\\
              \exp(-900) & \text{ if } \vect{x}_f = \vect{R}_{f2}\\
           \exp(-1000) & \text{ otherwise}
  \end{cases}
...
\section{Pattern-based higher order factors}
\label{sec:patternfunctions}
In this section, we seek a generic class of factors so that that it includes
forward sensor models described in Sec.~\ref{sec:forward} and that can be 
...
$P_f(.)$. Here, we take a consider a class of \emph{pattern-based} higher
order factors that are a generalization of those discussed by Komodakis et al
\cite{komodakis2009beyond}. These are factors of the form:
\begin{align}
  P'_f(\vect{x}_f) = \begin{cases}
    \psi_{m} & \text{ if $\vect{x}_f \sim \vect{R}_m$} \,\,\, \forall 1 \le m \le M\\
    \psi_{\text{min}} & \text{ otherwise}
  \end{cases}
  \label{eq:patternfunction}
\end{align}
where $\vect{R}_m$ is one of the mutually exclusive $M$ patterns and
expression $\vect{x}_f \sim \vect{R}_m$ denotes that vector $\vect{x}_f$
...
and they do not affect factor value $P_f(\vect{x}_f)$. A configuration
$\vect{x}_f$ ``matches`` pattern $\vect{R}_m$ if the state of \emph{fixed}
...
the term $\bar{z}_f(\vect{x}_f, g_f)$, just depends on first occupied cell. We
...
%   %&= \prod_{j \in n^m_0(f)}\bpmsg{j}{f}{R_{mj}}{r}\underbrace{\sum_{\vect{x}^*_f \in \Omega^*_f}\prod_{j \in n^m_*(f)}\bpmsg{j}{f}{x_j}{r}}_{=1\text{, because $\vect{R}_m$ do not contraints $n^m_*(f)$}}\\
...
\begin{align}
  \bpmsg{f}{i}{l_i}{r+1} &= \sum_{m \le M} \psi_m p(\vect{x_f} \sim \vect{R}_m)
