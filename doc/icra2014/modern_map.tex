...
The problem is to find the probability of all cells of the map being occupied given all observations till time $t$, $\measurements = [z_f]^\top_{1 \le f \le t}$ and $\rho = [\rho_f]^\top_{1 \le f \le t}$.
\begin{align}
...
\begin{align}
  \vect{x}* &= \argmax_{\map \in \Omega } p(\map | \measurements, \rho)
  \label{eq:mapproblem}
\end{align}
