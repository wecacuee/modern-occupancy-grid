...
desired values $\vect{r}^m$.
%, while the state of remaining \emph{free} nodes
...
minimizing the messages. Minimizing messages is trivial by as each term can be
...
sample space to find minimum value. Otherwise,
...
that does not match any of the patterns already considered.  For example, in
case of piecewise constant sensor model, there are only two patterns that need
...
To evaluate the convergence rate of each algorithm, we plot total energy of the
...
\cite{jojic2010accelerated} are step in this direction. Also, recent
...
decomposition class of methods.  However, even without using the state of art
optimum algorithms since we get better performance than sampling methods. This
only serves to prove our assertion that modern inference methods should be
used occupancy grid mapping when computation time is not a limiting factor. 
