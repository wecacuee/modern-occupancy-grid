...
Another common assumption is that the pose $g_t$ of the robot being independent of map $\map$:
\begin{align}
...
Note that neighborhood $n(i)$ of any variable node $i$ only consists of only
factors nodes, $n(i)\subseteq F \, \forall i \in V$ and vice versa.
Fig.~\ref{fig:factor-graph} shows factor
...
The dual decomposition algorithm employs theory of Lagrangian duals to find a
...
optimal assignment for variable nodes with disagreeing \emph{slaves} from the
messages:
\begin{align}
...
nodes is the same as desired values, $x_i = r^m_i \forall i \in n^m_0(f)$.
...
Using the messages as probability measure for the state of source nodes, 
%we can efficiently compute \eqref{eq:factor2node}. 
...
(Radish)~\cite{howard2003radish}. We thank Cyrill Stachniss for providing
this data.
...
As is evident from convergence comparison in
...
nodes. However, step size is a crucial parameter that affects speed of
