...
To overcome this limitation, Thrun \cite{thrun2003learning} proposes use of forward 
...
graph approach to occupancy grid mapping problem, which, to the
...
pattern-based factors in \cite{komodakis2009beyond} defines each pattern with 
...
called a factor graph \cite{kschischang2001factor}.  Note that any
neighborhood $n(i)$ of variable node only consists of factors, $n(i)\subseteq
F \, \forall i \in V$ and vice versa. Fig.~\ref{fig:factor-graph} shows factor
...
%The posterior over the factor graph, is 
%proportional to the product of functions $P_f(.)$ that depend only on the
%state of neighbors $\map_f$ of the corresponding factor node $f$:
%\begin{align}
%   P(\map) &= \frac{1}{Z}\prod_{f \in F} P_f(\map_f)
%  \label{eq:factorizationgeneric}
%\end{align}
%where $P_f(\map_f) \equiv p(z_f|\map_f, g_f)$ for occupancy grid mapping.
%
%
...
The dual decomposition algorithm employs theory of Lagrangian duals to find an
convex upper bound of the original combinatorial optimization problem.
%technique from integer programming to minimize the energy.
...
step size on \emph{cave} dataset in Fig.~\ref{fig:dualdecomposition-stepsize}. 
...
The message update equation \eqref{eq:factor2node} in BP algorithm and
the slave minimization \eqref{eq:slavemin} in DD algorithm are, in general,
...
In this section we describe consider two commonly used sensor models a)
...
from pose $g_f$. 
% Often the noise $\sigma$ in range measurements increases with
% the distance measured, lets 
% with distance as $\sigma = \sigma_0z_f$. 
...
%
% However, we chose to prefer free space over occupied ones.
Hence, we come with
the following factor that we use in all our experiments.
\begin{align}
  p(z_f|\map_f, g_f) &= \frac{1}{Z}\begin{cases}
...
compared to estimating it away from it.
...
% $n^m_*(f) = n(f) \setminus n^m_0(f)$
%can have any values from the label set 
%and they do not affect factor value $P_f(\map_f)$.
A configuration
$\map_f$ ``matches'' pattern $\vect{R}_m$ if the state of \emph{fixed}
nodes in same as desired values, $x_i = r^m_i \forall i \in n^m_0(f)$.
%For efficient computation we assume that number of patterns $M$ is much
%smaller than the sample space $\Omega_f$.

It is clear that PCSM \eqref{eq:piecewiseconstant} is an instance of pattern
based factors. It is also possible to represent GSM
...
\bpmsg{i}{f}{l_i}{r} = 1$.
%Thus, the messages are a valid probability measure for source variable node.
Using the messages as probability measure for the state of source nodes, we
%can efficiently compute \eqref{eq:factor2node}. 
...
We note that \emph{slave} function \eqref{eq:slavemin} is composed of two
terms, the factor itself and the messages. 
% If factor is a pattern based function, we can address
...
minimizing the messages. Minimizing messages is trivial as each term can be
minimized independently.

While minimizing the \emph{otherwise} case of patterns, we have to ensure the
exclusivity from explicit patterns.
%can be tricky as naive minimization can result in values that matches one of the explicit patterns.
% Depending on how big we expect the space of \emph{otherwise} case to be, we
% can approach it in two different ways. 
If the space for \emph{otherwise} case
is small (like in the gaussian sensor model), we simply go over the entire
space to find minimum value. Otherwise,
%we depend on the sample space covered by patterns being small.  In this case 
we keep finding n-best minimizations of messages until we get a minimization
that does not match any of the patterns already considered.  For example, 
for piecewise constant sensor model, there are only two patterns that need
to be checked for exclusion. The otherwise term can be easily minimized by
finding three best assignments that minimize the messages term. Note that
...
(negative log likelihood) of the graph with respect to CPU ticks taken by the
algorithm. The plots of energy convergence with respect to time for cave
dataset is shown in Fig.~\ref{fig:convergence-comparison}. These data clearly
show the improvement from moving to belief propagation and dual decomposition,
which, in all cases, lead to lower energies faster than our baselines.  DD
...
gaussian sensor model because of former being faster which is a consequence of
having fewer patterns in the pattern based factor formulation.
We have implemented the algorithms in C++ and the code will be released upon
the acceptance of the paper.

\subsection{Discussion}
As evident from convergence comparison in
...
algorithms, we see that the walls are thinner than corresponding results in
other algorithms which shows the inability of sampling based algorithms to
form lower energy thicker walls for piecewise constant sensor model. The
...
\cite{jojic2010accelerated} are steps in this direction. Also, recent
comparative study \cite{kappes2013comparative} points towards other candidate
methods, e.g. polyhedra based methodsa, that work better than dual
decomposition class of methods.  Even without using the state of art
inference algorithms, we get better performance than methods used so far. This
...
\begin{figure*}
  ...
  \caption{Qualitative results on different datasets. Each row represents a
    different dataset while each column represents a different algorithm. Th.e
  ...
  %2) hospital 
  %2) hospital section 
  2) albert-b. The grainy-ness in columns (3) and (4) is an artifact of
