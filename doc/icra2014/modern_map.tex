...
\section{Sum product}
% FIXME
%Pearls sum product used for error correcting codes is still widely used for computing marginals over factor graphs.
% FIXME
%Kschischang \cite{kschischang2001factor} explains  the algorithm nicely.
% 
...
\begin{figure*}
  ...
  \caption{Qualitative results at the end of 200sec for each algorithm (from left to right) a) Ground truth with the trajectory of the robot b) Belief Propagation c) Metropolis Hastings with heat map d) Metropolis Hastings without heat map e) Dual decomposition f) Two assumption algorithm}
