...
popular MCMC methods. The idea is to sample $\map$ from the target probability
distribution $P(\map)$ and after sufficient number of iterations,
...
For OG problem, 
%we need to sample map $\map$ from the probability distribution $P(\map)$.
we uniformly sample from all the cells in the map and flip the state of the
sampled cell to get the proposal point $\map'$. This procedure can be
